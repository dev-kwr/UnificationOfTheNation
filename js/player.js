...
} from './normalComboMotion.js?v=screen-safe-20260920e';
import { translateRibbonChains, resetRibbonChains } from './mobFx.js?v=screen-safe-20260920e';
import { CLOTH_CHAIN, HEADBAND_TAIL_SPEC, stepClothSwing, stepClothNode } from './clothChain.js?v=screen-safe-20260920e';
import { applyRendererMixin }    from './playerRenderer.js?v=screen-safe-20260920e';
import { applySlashTrailMixin }  from './playerSlashTrail.js?v=screen-safe-20260920e';
import { applySpecialMixin }     from './playerSpecial.js?v=screen-safe-20260920e';
import { applyShogunCombat }    from './shogunCombatHelper.js?v=screen-safe-20260920e';
import { applyDualComboMotion } from './dualComboMotion.js?v=screen-safe-20260920e';
import {
...
export class Player {
    ...
    handleInput(enemies = []) {
        ...
        // 忍具（Xキー）
        // 【一度離すまで次は撃たない】掛け金。タッチのボタンは指が少しずれると
        // 割り当てが外れて付け直る(applyTouchBinding)ため、1回のタップの中で
        // 押し直しが起きると justPressed が再び立ち、続けて飛んでしまう
        // (実機フィードバック 2026-08-11: 1タップで3枚繋がって飛ぶ。
        //  低フレームレートだと1フレームの間隔が広く、起きやすい)。
        // 押しっぱなしで連射する仕様は元から無いので、意図的な連打は影響を受けない。
        if (!input.isAction('SUB_WEAPON')) this.subWeaponInputReleased = true;
        if (!actionLocked && input.isActionJustPressed('SUB_WEAPON') && this.subWeaponInputReleased !== false) {
            this.subWeaponInputReleased = false;
            // クールダウン中は発動不可
            if (this.subWeaponTimer > 0) return;
            if (!this.currentSubWeapon) return;
