import { PLAYER, LANE_OFFSET } from './constants.js?v=screen-safe-20260810a';
import {
 ...
 *   - dims を素体フレーム(40x60)＋scaleMultiplier=SHOGUN_SCALE に設定し、ワールド身長で接地
...
export function applyShogunCombat(player) {
    ...
    // ================================================================
    // 初期化: 将軍 dims(素体40x60＋scale=SHOGUN_SCALE) 設定 → ネイティブ戦闘へ移行
    // ================================================================
    const initShogunInstances = (p) => {
        if (p._shogunInited) return;

        // 将軍は「スケールの違う忍者」: width/height は素体フレーム(40x60)を保持し、
        // ワールド寸法(=素体×scaleMultiplier=素体×SHOGUN_SCALE)は getWorldWidth/Height 経由で読む。
        p.width = SHOGUN_ACTOR_BASE_WIDTH;   // 40 素体
        p.height = SHOGUN_ACTOR_BASE_HEIGHT; // 60 素体
        p.actorBaseWidth = SHOGUN_ACTOR_BASE_WIDTH;
        p.actorBaseHeight = SHOGUN_ACTOR_BASE_HEIGHT;
        p.scaleMultiplier = SHOGUN_SCALE;
        p.speed = PLAYER.SPEED;
        // ワールド身長(=素体×SHOGUN_SCALE)で足を接地させる（startStage は忍者の高さで設定済み）
    ...
    // ネイティブ将軍は自分の currentSubWeapon をそのまま使うため、
