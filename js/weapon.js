...
import { GRAVITY, CANVAS_WIDTH, LANE_OFFSET, PLAYER } from './constants.js?v=screen-safe-20260815p';
import { audio } from './audio.js?v=screen-safe-20260815p';
import { SHOGUN_SCALE } from './shogunConstants.js?v=screen-safe-20260815p';
import { withDropShadow, drawSparks, drawBlastFlash, makeParticles, smoothstep01, pushTrailPoint, drawCometRibbon } from './weaponFx.js?v=screen-safe-20260815p';
...
function drawFirebombFuseCord(ctx, path, radius) {
    ...
    // 太さと色は HUD の生成png(images/hud_weapons/bomb.png)の実測に合わせる:
    //   幅  玉の直径の 1/37 前後 ＝ 0.055r。火縄は細い麻紐であって船の綱ではない。
    //   色  明部 rgb(247,212,155) の淡い麻色が主。影の rgb(166,86,36) は
    //       撚りの陰にだけ差す色で、これを主色にすると銅パイプに見える。
    // ゲーム内の玉は半径10px前後なので、下限を張って線が消えないようにする
    // (小さい時だけ相対的に太くなるが、読めないよりよい)。
    traceFuse();
    ctx.strokeStyle = '#241206';
    ctx.lineWidth = Math.max(1.7, r * 0.062);
    ctx.stroke();
    traceFuse();
