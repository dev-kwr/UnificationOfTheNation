...
import { CANVAS_WIDTH, SCREEN_WIDTH, CANVAS_HEIGHT, GRAVITY, GAME_STATE, STAGES, DIFFICULTY, OBSTACLE_TYPES, PLAYER, STAGE_DEFAULT_WEAPON, LANE_OFFSET, STAGE5_FLOOR, STAGE6_CORNER, UI_SIZE_ANCHOR, getDeviceProfile, setUiScaleFromFitScale, setCornerInsets, setNotchInsetX, setVirtualPadVisible, setBgmButtonVisible, isTouchOverlayMode, getFontScale } from './constants.js?v=screen-safe-20260920e';
import { BOSS_STAGING } from './bossStaging.js?v=screen-safe-20260920e';
import { isUpdateAvailable, applyUpdate, checkForUpdate } from './appUpdate.js?v=screen-safe-20260920e';
import { getStageSelectLayout, renderStageSelect, STAGE_SELECT_ORDER } from './stageSelect.js?v=screen-safe-20260920e';
import { clearFilteredImageCache, getFilteredImageCacheStats } from './filteredImage.js?v=screen-safe-20260920e';
import { BonusStage, BONUS_STAGE_IMAGES } from './bonusStage.js?v=screen-safe-20260920e';
import { TrainingStage, TRAINING_STAGE_IMAGES } from './trainingStage.js?v=screen-safe-20260920e';
import { sideBestKey, normalizeSideBests, getSideBest } from './sideStageCommon.js?v=screen-safe-20260920e';
import { preloadImages, areImagesSettled, getImagesProgress, releaseImagesExcept } from './imageCache.js?v=screen-safe-20260920e';
import { readPhysicalScreen, computeScreenWidth } from './screenGeometry.js?v=screen-safe-20260920e';
import { input } from './input.js?v=screen-safe-20260920e';
...
import { Player } from './player.js?v=screen-safe-20260920e';
import { createSubWeapon } from './weapon.js?v=screen-safe-20260920e';
import { Stage, preloadStageImages, prefetchStageImages, areStageImagesSettled, getStageImagesProgress, releaseStageImagesExcept } from './stage.js?v=screen-safe-20260920e';
import { GRAPPLE_PHASE } from './stage6Grapple.js?v=screen-safe-20260920e';
import { UI, renderTitleScreen, renderTitleDebugWindow, renderGameOverScreen, renderStatusScreen, renderStageClearAnnouncement, renderLevelUpChoiceScreen, getLevelUpChoiceLayout, renderSideResultScreen, getSideResultLayout, renderPauseScreen, getPauseButtons, renderGameClearScreen, renderIntro, renderEnding, getTitleScreenLayout, getStatusScreenLayout, getTitleDebugLayout, getUpdateModalLayout, renderBossNameBanner, getBossNameBannerBox } from './ui.js?v=screen-safe-20260920e';
import { CollisionManager, checkPlayerEnemyCollision, checkEnemyAttackHit } from './collision.js?v=screen-safe-20260920e';
import { saveManager } from './save.js?v=screen-safe-20260920e';
import { shop } from './shop.js?v=screen-safe-20260920e';
import { audio } from './audio.js?v=screen-safe-20260920e';
import { ShadowRenderer } from './shadow.js?v=screen-safe-20260920e';
import { applyShogunCombat } from './shogunCombatHelper.js?v=screen-safe-20260920e';
import { getRockVisualPalette } from './obstacle.js?v=screen-safe-20260920e';
...
// 【4秒では足りない】。1ステージの背景は10MB前後あり、携帯回線では10秒を超える
// ことが珍しくない。4秒で打ち切ると「開始はしたが絵は進みながら出てくる」に
// なる(実機フィードバック 2026-09-19)。打ち切りは事故のための最後の逃げ道と
// 割り切り、通常は必ず揃うまで待つ。待たせる間は下の表示で進捗を見せる。
const STAGE_ASSET_WAIT_MAX_MS = 45000;
// 待ちの表示を出し始めるまでの猶予(ms)と、出るまでのフェード(ms)。
// 一瞬で終わる待ちに文字を出すと暗転がちらつくので、長引いたときだけ出す。
const STAGE_ASSET_HINT_DELAY_MS = 900;
const STAGE_ASSET_HINT_FADE_MS = 320;
...
// ポーズUI(下端のボタン列)の開閉の遷移時間(秒)。ESC連打の妨げにならない短さ。
const PAUSE_OPEN_ANIM_SEC = 0.12;
...
// Stage3 の自動スクロールから追従への受け渡しで、カメラが追いつく速さの上限。
// 自動スクロール中のプレイヤーは画面右端に張り付いているので、いきなり
// 「プレイヤーを画面中央へ」にすると一気に寄る(実測で598px飛んだ)。
// 走り(360px/s)より十分速く、かつ一瞬で飛ばない値。
const STAGE3_HANDOVER_CATCHUP_SPEED = 1200;
// 終盤(峠を抜けて道が開ける区間)で自動スクロールを何倍まで速めるか。
const STAGE3_APPROACH_SPEED_MULT = 2.2;
// デバッグ(?at=finale)でボス部屋の何画面手前から始めるか。
// Stage3 だと 3.6 画面手前 ≒ progress 7400 で、ちょうど城下町が画面へ入り
// 始める場所。そこから「町のフレームイン → 山が沈む → 加速 → 鳥居 →
// 足元が土の道へ → ボス部屋」の全部が続けて見られる。
const DEBUG_FINALE_SCREENS = 3.6;
...
class Game {
    constructor() {
        ...
        // ポーズ自体の開閉の遷移量(0..1)。解除後も 0 になるまで描き続ける
        this.pauseAnim = 0;
        // ポーズ中に「地図に戻る」を出していたか。閉じ際の描画でも同じ顔ぶれを使う
        // （解除で canPauseReturnToMap が false に落ちるため、latch しないと
        //   フェードアウト中にボタンが1つ消えて残りが横に飛ぶ）。
        this.pauseCanGoMap = false;
        this.pauseMenuIndex = 0;   // ポーズのカーソル(キーボード用)
        this.lastTime = 0;
        this.deltaTime = 0;
        ...
        this.updateModalAnim = 0;       // 更新モーダルの登場の遷移量(0..1)
        ...
        this.debugFinaleStart = false; // デバッグ用：終盤(ボス部屋の手前)から開始
     ...
     *   ?stage=3&at=finale   … 終盤(ボス部屋の手前 DEBUG_FINALE_SCREENS 画面)から
     *   ?stage=3&at=finale&screens=2  … 手前を何画面ぶんにするか(0.5〜8)
    ...
    getDebugStartPointFromUrl() {
        try {
            ...
            if (raw === 'finale' || raw === 'end' || raw === 'last' || params.get('finale') === '1') return 'finale';
            return null;
        } catch {
            return null;
        }
    ...
    getDebugFinaleScreensFromUrl() {
        try {
            const raw = new URLSearchParams(window.location.search).get('screens');
            if (raw === null) return DEBUG_FINALE_SCREENS;
            const n = Number.parseFloat(raw);
            if (!Number.isFinite(n)) return DEBUG_FINALE_SCREENS;
            return Math.max(0.5, Math.min(8, n));
        } catch {
            return DEBUG_FINALE_SCREENS;
        }
    }
    ...
    continueGame(saveData) {
        ...
        // 武器作成関数をインポート
        import('./weapon.js?v=screen-safe-20260920e').then(module => {
            // 基本ステータス復元
    ...
    startStage() {
        ...
        // ─── デバッグ：終盤(ボス部屋の手前)からスタート ───────────────
        // ボス部屋そのものではなく【そこへ至る道】を見るための開始地点。
        // Stage3 なら「城下町のフレームイン → 山が地平へ沈む → 加速 →
        // 峠の鳥居 → 足元が土の道へ → ボス部屋 → 会敵」が続けて確認できる。
        // 道中の山場(中ボス)と雑魚は、終盤の絵の検証を邪魔するので省く。
        if (this.debugFinaleStart) {
            this.debugFinaleStart = false;
            const bossRoomLeft = Math.max(0, this.stage.maxProgress - CANVAS_WIDTH);
            const back = CANVAS_WIDTH * this.getDebugFinaleScreensFromUrl();
            const camX = Math.max(0, bossRoomLeft - back);
            this.scrollX = camX;
            this.stage.progress = camX;
            this.stage.lastProgress = camX;
            this.stage.midBossSpawned = true; // 中ボスはスキップ
            this.stage.enemies = [];          // 雑魚敵をクリア(湧きは進行に沿って再開する)
            this.ensurePlayerDimsReady();
            // Stage3 は自動スクロールでカメラが先へ進むので、プレイヤーは
            // 少し左に置く(本番の「追いかける」間合いと同じ)。他は画面中央。
            const screenRatio = this.currentStageNumber === 3 ? 0.38 : 0.5;
            this.player.x = camX + CANVAS_WIDTH * screenRatio;
            this.player.groundY = this.stage.groundY;
            this.player.y = this.stage.groundY + LANE_OFFSET - this.player.getWorldHeight();
            this.player.vx = 0;
            this.player.vy = 0;
    ...
    update() {
        ...
        // ポーズUIの開閉の遷移量。updatePaused では【解除後に呼ばれない】ので、
        // 状態に依らずここで進める（ここが唯一の書き手）。
        this.advancePauseAnim();
        switch (this.state) {
    ...
    updateTitle() {
        ...
        // 更新モーダルの登場。検知は非同期に立つので、フラグを遷移量へ変換する。
        {
            const step = this.deltaTime / TITLE_DEBUG_ANIM_SEC;
            const target = isUpdateAvailable() ? 1 : 0;
            this.updateModalAnim = this.updateModalAnim < target
                ? Math.min(target, this.updateModalAnim + step)
                : Math.max(target, this.updateModalAnim - step);
        }
    ...
    startNewGame() {
        ...
        this.debugFinaleStart = this.debugStartPoint === 'finale';
        ...
        if (this.debugFinaleStart) { this.debugBossRoomStart = false; this.debugStage6Corner3Start = false; }
        // Stage5(階層)・Stage6(層)は横一本の世界ではないので「手前○画面」が
        // 意味を持たない。それぞれの既存の開始地点へ振り替える。
        if (this.debugFinaleStart && this.currentStageNumber === 6) {
            this.debugFinaleStart = false;
            this.debugStage6Corner3Start = true;
        } else if (this.debugFinaleStart && this.currentStageNumber === 5) {
            this.debugFinaleStart = false;
            this.debugBossRoomStart = true;
        }
        this.player = new Player(100, this.groundY - PLAYER.HEIGHT, this.groundY);
    ...
    initStage(stageNum) {
        ...
        // 背景と共通アセットの読み込みをここで始める。新規開始は口上(INTRO)を挟むので、
        // その数秒をまるごとロードに使える（開始要求時の待ちがそのぶん短くなる）。
        preloadStageImages(stageNum);
    ...
    getStage3ApproachSpeedMult() {
        const s = this.stage;
        if (!s || !Number.isFinite(s.maxProgress)) return 1;
        const from = s.maxProgress - CANVAS_WIDTH * 2.6;
        const to = s.maxProgress - CANVAS_WIDTH;   // ボス部屋の画角
        if (!(to > from)) return 1;
        const t = Math.max(0, Math.min(1, (this.scrollX - from) / (to - from)));
    ...
    isStage3AutoScrollActive() {
        const s = this.stage;
        return this.isMainStage(3) && !s.bossSpawned;
    }
    ...
    stepStage6ArenaEntryArc(advance) {
        ...
        this.player.facingRight = true;
        // 【運ばずに物理で追わせる】。差分を全ノードへ足すと帯が本体と同じだけ
        // 動いて遅れが消える。飛び込みは瞬間移動ではないので、根元を今の頭に
        // 置いて物理を回せば、跳び上がり・落下に遅れて垂れる形になる。
    ...
    updateStage6GrappleClimb() {
        ...
        // 【運ばずに物理で追わせる】。以前は本体の移動差分を全ノードへそのまま
        // 足していたが、それだと帯が本体と同じだけ動く＝遅れが出ず、慣性が
        // 完全に消える(実機フィードバック 2026-08-17「物理を感じない」)。
        // 引き上げは瞬間移動ではないので、根元を今の頭に置いて物理を回せば、
        // 節が遅れて下へ垂れる正しい形になる(距離拘束があるので伸び切らない)。
        if (typeof this.player.updateVisualClothDuringCinematic === 'function') {
            this.player.updateVisualClothDuringCinematic(this.deltaTime);
        }
    ...
    updatePlaying() {
        ...
        // --- スクロール処理 ---
        const stage3AutoScrollActive = this.isStage3AutoScrollActive();
        const screenCenter = CANVAS_WIDTH / 2;
        const cameraBeforeScroll = this.scrollX;

        if (stage3AutoScrollActive) {
            this.scrollX += this.stage3AutoScrollSpeed * this.getStage3ApproachSpeedMult() * this.deltaTime;
        } else if (this.isMainStage(5)) {
        ...
        // 【Stage3のカメラは速さに上限を掛ける】。自動スクロール中はプレイヤーが
        // 画面右端に張り付くので、他ステージのように「歩いた分だけカメラが付いて
        // くる」状態にならない。そのままだとボス部屋の寄せが差分いっぱいに走り、
        // 会敵の直前だけカメラが一気に流れる(実測 76px/frame)。
        // 上限を掛けると、飛ぶのではなく寄る絵になる(600pxで約0.5秒)。
        if (this.isMainStage(3) && !this.stage.bossDefeated) {
            const dt = Math.max(0, Math.min(1 / 30, this.deltaTime));
            const maxStep = STAGE3_HANDOVER_CATCHUP_SPEED * dt;
            if (this.scrollX - cameraBeforeScroll > maxStep) {
                this.scrollX = cameraBeforeScroll + maxStep;
            }
        }
        ...
        for (const obs of obstacleList) {
            ...
            if (this.rectIntersects(this.player, obs)) {
                if (obs.type === OBSTACLE_TYPES.SPIKE) {
                    ...
                    if (sideCollision) {
                        // 自動スクロールが効いている窓と同じ条件で判定する
                        // (カメラが進まなくなった後は挟まれようがない)。
                        const stage3AutoScrollCrushActive = this.isStage3AutoScrollActive();
    ...
    updatePaused() {
        ...
        if (input.touchJustPressed) {
            const tx = input.lastTouchX;
            const ty = input.lastTouchY;
            const hit = getPauseButtons({ canGoMap: this.pauseCanGoMap }).find(
        ...
        /* 【キーボードでも選べるようにする】。クリック(タップ)でしか押せず、
           PCでは手をマウスへ移さないと何も選べなかった(実機フィードバック 2026-09-20)。
           ←→ で選び、決定は画面のボタンと同じ「2回で確定」に揃える。 */
        const btns = getPauseButtons({ canGoMap: this.pauseCanGoMap });
        if (btns.length > 0) {
            const moved = input.isActionJustPressed('LEFT') ? -1
                : input.isActionJustPressed('RIGHT') ? 1 : 0;
            if (moved !== 0) {
                this.pauseMenuIndex = (this.pauseMenuIndex + moved + btns.length) % btns.length;
                this.pauseArmedId = null;   // 選び直したら確認は振り出しへ
                audio.playCursor();
            }
            if (input.isActionJustPressed('CONFIRM')) {
                input.consumeAction('CONFIRM');
                const hit = btns[Math.min(this.pauseMenuIndex, btns.length - 1)];
                if (hit) {
                    if (typeof audio.playSelect === 'function') audio.playSelect();
                    if (hit.id === 'resume') {
                        this.resumeFromPause();
    ...
    /** ポーズUIの開閉の遷移量を進める。開く/閉じるで同じ速さ。 */
    advancePauseAnim() {
        const step = this.deltaTime / PAUSE_OPEN_ANIM_SEC;
        const paused = this.state === GAME_STATE.PAUSED;
        const target = paused ? 1 : 0;
        this.pauseAnim = this.pauseAnim < target
            ? Math.min(target, this.pauseAnim + step)
            : Math.max(target, this.pauseAnim - step);
        // ボタンの顔ぶれはポーズ中に決めて閉じ切るまで固定する
        if (paused) this.pauseCanGoMap = this.canPauseReturnToMap();
        else this.pauseMenuIndex = 0;   // 閉じたら次に開くときは先頭から
    }
    ...
    /* キーボードで選んでいるポーズボタン。タッチはタップで直接押すので出さない。 */
    getPauseFocusId() {
        if (isTouchOverlayMode()) return null;
        const btns = getPauseButtons({ canGoMap: this.pauseCanGoMap });
        if (btns.length === 0) return null;
        return (btns[Math.min(this.pauseMenuIndex, btns.length - 1)] || {}).id || null;
    }

    /* 【本編の第N階層を遊んでいるか】。寄り道(小判蔵/修行道場)は本編の進行
       (currentStageNumber)を変えない作りなので、番号だけで見ると寄り道の最中にも
       本編用の処理が走る。第五階層まで進んでから蔵や道場へ入ると、階段の処理が
       寄り道の Stage を叩いて毎フレーム落ちていた
       (getStairClimbProgress is not a function → update で例外 → render へ
        辿り着かず、音だけ鳴って画面が真っ黒。実機ログ 2026-09-20)。 */
    isMainStage(n) {
        return this.currentStageNumber === n && !!this.stage && !this.stage.sideKind;
    }

    /**
     * ポーズに「地図に戻る」を出してよい場面か。
     * 場を遊んでいる時なら本編でも寄り道でも出す。セレクト/ステータスからの
     * ポーズだけは、戻る先が今そこなので要らない。
     * 【寄り道でも出す】。刻限(60秒)を待たないと抜けられず、入り直しも
     * 効かなかった(実機フィードバック 2026-09-19)。途中で抜けた回は記録に
     * 残らない＝結果発表を通らないので、スコアの更新もセーブもされない。
     */
    canPauseReturnToMap() {
    ...
    updateShop() {
        ...
        if (!shop.isOpen) {
            if (this.returnToStageClearAfterShop) {
                // 幕間へ戻るのも暗転で切る。shop.render は isOpen で早期returnするので、
                // 幕が降りている間だけ開いた扱いに戻して絵を残し、底で本当に閉じる。
                shop.isOpen = true;
    ...
    startStageTransition() {
        ...
        /* 寄り道へ向かうときは、先に本編の絵を手放してから読む。20MB前後を抱えた
           まま10MB強を足すと、iOS ではメモリが足りずに読み込みごと落ちることが
           あり、そのとき音だけ鳴って画面が真っ黒の場が始まる
           (実機フィードバック 2026-09-20)。 */
        if (this.pendingTrainingStart) {
            releaseImagesExcept(TRAINING_STAGE_IMAGES);
            preloadImages(TRAINING_STAGE_IMAGES);
        } else if (this.pendingBonusStart) {
            releaseImagesExcept(BONUS_STAGE_IMAGES);
    ...
    requestStageStart() {
        ...
        if (this.pendingBonusStart) {
            ...
            releaseImagesExcept(BONUS_STAGE_IMAGES);   // 本編の絵を抱えたまま読まない
            preloadImages(BONUS_STAGE_IMAGES);
    ...
    updatePendingStageStart() {
        ...
        if (pending.side) {
            ...
            pending.progress = getImagesProgress(images);
    ...
    // 背景アセット待ちの表示。暗幕の上に、いちばん静かな形で「止まっていない」
    // ことだけを伝える。短い待ちには出さない（暗転がわずかに伸びるだけのほうが
    // 場が乱れない）ので、長引いたときだけ薄く浮かせる。
    renderStageAssetWait() {
        const pending = this.pendingStageStart;
        if (!pending) return;
        const elapsed = pending.waitMs - STAGE_ASSET_HINT_DELAY_MS;
        if (elapsed <= 0) return;
        const alpha = Math.min(1, elapsed / STAGE_ASSET_HINT_FADE_MS);
        const progress = pending.progress;
        const total = progress?.total || 0;
        const ratio = total > 0 ? Math.min(1, progress.loaded / total) : 0;

        const ctx = this.ctx;
        const fs = getFontScale();
        const cx = SCREEN_WIDTH / 2;
        const barW = Math.round(260 * fs);
        const barH = Math.max(2, Math.round(2 * fs));
        const barY = Math.round(CANVAS_HEIGHT * 0.62);
        const textY = barY - Math.round(18 * fs);
        ...
        ctx.font = `500 ${Math.round(17 * fs)}px "Zen Old Mincho", serif`;
        ctx.fillText('読込中', cx, textY);
        // 下地(読めていない分)と、読めた分。線1本だけに留めて場を汚さない。
        ctx.fillStyle = 'rgba(214, 203, 178, 0.18)';
        ctx.fillRect(cx - barW / 2, barY, barW, barH);
        if (ratio > 0) {
            ctx.fillStyle = 'rgba(201, 185, 143, 0.85)';
            ctx.fillRect(cx - barW / 2, barY, Math.round(barW * ratio), barH);
        }
        ctx.restore();
    }
    ...
    updateSideResult() {
        ...
        const decide = (id) => {
            ...
            if (id === 'retry') {
                this.retrySideStage(kind);
            } else {
                /* 画面の暗転と同じ長さで曲も落とす。ここで落としておかないと、
                   地図の曲を出すときに寄り道の曲が切られ、地図の曲が鳴り出すまで
                   無音になる(実機フィードバック 2026-09-19)。
                   地図側の playBgm がフェードインで重ねるのでクロスフェードになる。 */
    ...
    handleStageClearConfirm() {
        ...
        if (this.stageClearMenuIndex === 2) {
            ...
            // よろず屋。独立画面へ丸ごと差し替わるので暗転を挟む
            // （幕の裏で入れ替える＝地図⇔ステータスと同じ作法）。
            audio.playSelect();
            this.startScreenFade(() => {
                if (this.player) this.player.previewMode = false;
                shop.open();
    ...
    render() {
        ...
        switch (this.state) {
            case GAME_STATE.TITLE:
                renderTitleScreen(this.ctx, this.difficulty, this.titleMenuIndex, this.hasSave, this.updateModalAnim);
            ...
            case GAME_STATE.PAUSED:
                ...
                renderPauseScreen(this.ctx, {
                    canGoMap: this.pauseCanGoMap,
                    focusId: this.getPauseFocusId(),
                    anim: this.pauseArmedAnim,
                    openT: this.pauseAnim
                });
                break;
        ...
        // ポーズを解いた後の余韻。解除した瞬間に消さず、遷移量が 0 になるまで
        // 再開後の画面の上でボタンを薄れさせる（顔ぶれは pauseCanGoMap で固定）。
        if (this.state !== GAME_STATE.PAUSED && this.pauseAnim > 0.001) {
            renderPauseScreen(this.ctx, {
                canGoMap: this.pauseCanGoMap,
                focusId: this.getPauseFocusId(),
                anim: this.pauseArmedAnim,
        ...
        // ＝スピーカーごと沈める(renderPlaying内で描くとボタンだけ明るく残る)。
        if ((this.state === GAME_STATE.PLAYING || this.state === GAME_STATE.PAUSED)
            && (this.currentStageNumber === 5 || this.currentStageNumber === 6)
            && this.stage && this.stage.isFloorTransitioning) {
            this.stage.renderFloorTransition(this.ctx);
        }
    ...
    renderPlaying(playerAlpha = 1.0, forceStanding = false) {
        ...
        const grappling = this.isMainStage(6) && this.stage.isStage6Grappling();
        ...
        // Stage5/6 のフロア/角遷移の暗転はここでは描かない。BGMボタンより
        // 後ろ(render()末尾の幕の列)で描かないと、ボタンだけ暗転せずに残る
