...
### フェーズ2: ボーナス/修行ステージ
- ボーナス(小判): 敵なしの短い横スクロール。小判を拾って駆け抜ける(時間 or 距離で終了→セレクトへ戻る)。
  解放: Stage2 クリア後。再入場可(回数制限やクールダウンは調整枠)。
- 修行(道場): 敵が波状に湧く固定アリーナ。任意離脱(ポーズから「切り上げる」)。経験値・熟練稼ぎ。
  解放: Stage3 クリア後。
- どちらも `Stage` を継承した軽量ステージ(背景1枚+地面のみ、ボス無し、クリア概念無し=離脱で終了)。
- セレクトから入る際もステータス画面(装備確認)を挟む。
