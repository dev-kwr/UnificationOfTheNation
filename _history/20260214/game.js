...
class Game {
    ...
    updateExpGems() {
        ...
        // 少し浮かせる
        const groundLimit = this.groundY - 12;
        ...
        this.expGems = this.expGems.filter((gem) => {
            ...
            gem.y += gem.vy;
            // 明滅アニメーションの進捗
            gem.sparklePhase += this.deltaTime * 6.5;

            if (gem.y > groundLimit - gem.size * 0.6) {
                gem.y = groundLimit - gem.size * 0.6; // 地面に刺さりすぎないように補正
                gem.vy *= -0.2;
                gem.vx *= 0.82;
    ...
    renderExpGems(ctx) {
        ...
        for (const gem of this.expGems) {
            ...
            // 色味の強化：濃いエメラルドグリーンと濃紺（サファイア）
            // rgbaの文字列ではなく、数値として保持し、alphaを動的に変える
            const baseRGB = isBossGem ? '20, 40, 180' : '0, 180, 80';
            
            // 明滅（transition）: 1.5秒周期で 0.6 〜 1.0 の間を揺らぐ
            const idlePulse = (Math.sin(now * 0.008) + 1) * 0.2 + 0.6;
            
            // 消滅直前の高速点滅ロジック（残り1.5秒以下で開始）
            let alpha = idlePulse;
            if (gem.lifeMs < 1500) {
                // 残り時間に応じて点滅速度を上げる (8Hz -> 16Hz)
                const blinkFreq = gem.lifeMs < 600 ? 0.04 : 0.02; 
                if (Math.floor(now * blinkFreq) % 2 === 0) {
                    alpha = 0.1; // 消灯
                }
            }
            ...
            // 揺らぎ
            const sway = Math.sin(gem.sparklePhase) * 2;
            ctx.translate(0, sway);
            ctx.globalAlpha = alpha;

            // ボスジェム（サファイア）は通常(緑)の1.3倍程度
            const radius = isBossGem ? gem.size * 1.3 : gem.size * 1.0;

            // 共通形状：「内側が湾曲した4点スター」
            const innerRadius = radius * 0.25; 
            
            const drawStar = (r, inR) => {
                ctx.beginPath();
                ctx.moveTo(0, -r);
                ctx.quadraticCurveTo(inR, -inR, r, 0);
                ctx.quadraticCurveTo(inR, inR, 0, r);
                ctx.quadraticCurveTo(-inR, inR, -r, 0);
                ctx.quadraticCurveTo(-inR, -inR, 0, -r);
                ctx.closePath();
            };

            // フラットかつエナジーを感じる表現：高彩度・明度差で光を表現

            // 1. ベース（高彩度で濃いめ）
            // 緑: Deep Vivid Green / 青: Deep Vivid Blue
            const baseColor = isBossGem ? '#1040d0' : '#00a040';
            drawStar(radius, innerRadius);
            ctx.fillStyle = baseColor;
            ctx.fill();
            
            // 2. 輪郭（ネオンカラーでエナジー感）
            // 青: Cyan-ish / 緑: Neon Green
            ctx.strokeStyle = isBossGem ? '#40a0ff' : '#00ff80';
            ctx.lineWidth = 1.5;
            ctx.stroke();

            // 3. コア（白っぽくせず、同系色の明るい色でフラットに重ねる）
            const highlightRadius = radius * 0.55;
            const highlightInner = innerRadius * 0.55;
            drawStar(highlightRadius, highlightInner);
            const coreColor = isBossGem ? '#3060f0' : '#20c060';
    ...
    updateDefeat() {
        ...
        if (this.player) {
            // 徐々に上へ昇っていく（昇天演出）
            this.player.vx = 0; 
            // 物理演算（update）を呼ぶと重力で落ちてしまうため、単純に y を減算する
            this.player.y -= this.deltaTime * 60 * 1.5; // ゆっくり上昇
    ...
    render() {
        ...
        switch (this.state) {
            ...
            case GAME_STATE.DEFEAT:
                // プレイ画面を背景に描画（昇天中）
                // プレイヤーは forceStanding: true で直立不動
                this.renderPlaying(1.0, true);
                
                // 赤フェード演出: 徐々に視界が赤く染まる
                // playerDefeatTimer: 2200 -> 0
                // 最初は透明(alpha 0) -> 最後は真っ赤(alpha 1.0)
                const maxDuration = 2200;
                // タイマーが減るにつれてalphaが増える
                const progress = Math.max(0, Math.min(1, 1 - (this.playerDefeatTimer / maxDuration)));
                // 指数関数的に濃くする（後半急激に赤くなる）
                const redAlpha = Math.pow(progress, 2); 
                
                this.ctx.fillStyle = `rgba(180, 0, 0, ${redAlpha})`;
                this.ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
                break;
            case GAME_STATE.LEVEL_UP:
                this.renderPlaying();
    ...
    beginPlayerDefeat(sourceX = null) {
        ...
        // 演出時間を短縮 (3000ms -> 2200ms)
        this.playerDefeatTimer = 2200;
