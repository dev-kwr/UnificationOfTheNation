...
class AudioManager {
    constructor() {
        ...
        this.bgmFiles = {
            ...
            stage_odd: 'bgm/stage.mp3',   // 奇数ステージ (1, 3, 5...)
            stage_even: 'bgm/stage2.mp3'  // 偶数ステージ (2, 4...)
        };
    }
    ...
    playBgm(type = 'stage', stageNum = 1) {
        // 同じタイプなら再生し直さない
        if (this.currentBgmType === type && type !== 'stage') return;
        
        let filePath = '';
        
        if (type === 'stage') {
            // ステージ番号で切り替え（奇数：stage, 偶数：stage2）
            const isOdd = stageNum % 2 !== 0;
            const targetType = isOdd ? 'stage_odd' : 'stage_even';
