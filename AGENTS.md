...
5. **画面端の退避は「端末の角丸・ノッチに実際に被る要素だけ、回避に必要な最小限」**（ユーザー確定方針 2026-08-09）:
   - 判断基準はこれだけ。**被らない位置にある要素を予防的に内側へ寄せないこと**。下部の仮想パッド（スティック/ポーズ/攻撃/忍具/奥義/転身）は元位置（下140・横150論理px）が既に十分内側で被らないため退避なし＝従来配置を維持する（`getPadLayout`。左右はノッチ実値(env)のみ考慮）。一律退避を適用して「元の位置で問題なかった」と実機フィードバックで差し戻された経緯があるので、勝手に再適用しないこと。
   - 実際に被る要素（HUD・BGMボタン・ステージ名・タイトル⚙・操作説明行＝画面端から実寸十数css-pxのUI）は `getScreenSafeArea()` を単一導出として退避する。角丸の推定は `game.js` の `CORNER_RADIUS_RATIO` / `CORNER_CLEARANCE_RATIO`（詰まり具合の調整はここだけ。幾何最小0.293Rに曲面余裕を足した0.38R＝必要最小限を超えて増やさない）。
