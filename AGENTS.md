...
6b. **ステージ間の遷移は「クリア演出 → セレクト → ステータス → 開始」**:
   ...
   - **無双の密度では敵に横の押し退けが要る**(`spreadEnemies`)。全員が同じ的へ真っ直ぐ寄るので、そのままだと1点に重なって団子になる。位置だけを最小限ずらし、AIや速度には触れない。押し退け量は**AIの寄せ足(1〜2px/frame)より強く**しないと押し負ける。あわせて足場コライダーを `enemy.update` へ渡し(渡さないと台の上が安全地帯になる)、上にいるプレイヤーへはたまに跳ばせる(`chasePlayerUpward`)。
   - **寄り道の足場は【床に据えた道具】にする**。壁付けの棚は板が手前へ張り出す＝奥行きが要る絵になり、投影文法(壁は立面・床は俯瞰の帯)と食い違う(実機フィードバック 2026-08-11)。絵は「上端＝天面(＝当たり判定の面)／下端＝床に接する線」で切り詰めておけば、`p.y` と足元(`PROP_FOOT_Y`＝groundY+LANE_OFFSET)へそのまま収まる。幅は絵の縦横比から出す(`scratch/fit_dojo_props.py`)。引き伸ばすと木目が寝る。
   - **背景の一枚絵から【床は使わない】**。地平線より下は本編と同じ俯瞰の床帯なので、遠近の付いた床を焼き込んだ絵をそのまま敷くと他ステージと奥行きが食い違う(実機フィードバック 2026-08-11)。絵は床境界(`FLOOR_LINE_V`)の上＝壁だけを切り出して地平線へ合わせ、板の間は描画側(`renderFloorBand`)が水平な板目だけで敷く。`FLOOR_LINE_V` は絵を差し替えるたびに実測すること(板目の継ぎ目が始まる行を探す)。
   - **固定画面の背景は幅を CANVAS_WIDTH へ1:1で描く**(cover で切らない)。そうすると絵の中の柱や長押の x をワールド座標へそのまま読み替えられ(`x_world = x_image * 1280/絵の幅`)、足場をその上へ載せられる。

6c. **手書きSVGアセットの作法**:
   - **`width`/`height` 属性を必ず書く**。viewBox だけの SVG を `new Image()`→`drawImage` で描くと、Chrome が内在寸法を 300x150 とみなして**絵が歪む**(火薬玉の球が横長の楕円になった)。`width="672" height="672"` のように viewBox と同じ実寸を明示する。
   - **XMLコメントに連続ハイフン(`--`)を書かない**。区切り線のつもりで `----` を入れると SVG が壊れ、画像が読めずコード側のフォールバックに落ちる(黙って絵が変わるので気づきにくい)。編集したら `python3 -c "import xml.etree.ElementTree as ET; ET.parse('images/xxx.svg')"` で検算する。
   - 動きのある部分(導火線の揺れ・火花の明滅)は SVG に焼き込まず、ゲーム側のコードで重ねる。静止画に火を焼き込むと「消えない火花」になる。
   - **生成pngの写しを作るなら、目分量ではなく実測して数値で合わせる**。png をピクセル走査して「部材の幅/高さ/位置を半径Rの倍数で」出し、SVG側の座標をその値から決めて、根拠をSVGのコメントへ残す。火薬玉では口金の突き出し・導火線の太さ・しなりの式(`横ずれ = 0.35h² + 0.15h`)をこの方法で合わせた。
   - **曲線の制御点に、曲線上の値をそのまま置かない**。ベジェは制御点を通らないので、実測どおりの位置に置くと根元から傾いた「ほぼ直線」になる。t=1/3, 2/3 での値が実測に一致するよう逆算する。
   - **陰影を帯(矩形)で描かない**。口金の中ほどへ暗い矩形を敷いたら「この水平線は何」と差し戻された。面はグラデーションで作る。同じ理由で、光は必ず**放射グラデーション＋半径の2倍以上の矩形**で描く(線形グラデーションを細い矩形へ流すと縁が硬い長方形として残る)。
...
6e. **画像は `drawImageGraded()` で描く（`ctx.drawImage` を直接呼ばない）**:
   - `ctx.filter` 付きの `drawImage` は毎フレーム全画素を通す遅い経路。実測(2026-08-11・第4階層)で**1フレーム17〜18msのうち約13ms**を占めており、低スペック端末の処理落ちの主因だった(毎フレーム12回・約277万画素)。
   - `js/filteredImage.js` の `drawImageGraded(ctx, img, ...)` は、`ctx.filter` が掛かっているときだけフィルタ済みキャンバスを焼いてキャッシュし、以後は素で貼る。**`filter` が `none` のときは素通し**なので、フィルタを使わない描画にも無条件で使ってよい(掛け忘れ・外し忘れの事故が起きない)。
   - **焼くのは「描画される寸法」で**。元寸で焼いて縮小して貼ると、縮小の再標本化が画像ソースからキャンバスソースへ変わって画質が落ちる(倍率0.114の層で平均差10.9・画素の48%が差8超)。目標寸法まで落として焼き、等倍で貼れば**画素完全一致**になる。
   - `blur`/`drop-shadow` は焼かない(掛ける寸法でボケ幅が変わる)。拡縮が動く層も焼かない(毎フレーム焼き直すと逆に遅い)。
   - キャッシュはステージ開始時に `clearFilteredImageCache()` で捨てる(先読みは1ステージ先までという設計に合わせる)。
   - 実測: 第4階層 17.0〜18.4ms → **3.7ms**。第1・第6階層も「フィルタを完全に無効化した場合」と同じ水準まで落ちた。
