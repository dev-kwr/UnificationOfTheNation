...
6b. **ステージ間の遷移は「クリア演出 → セレクト → ステータス → 開始」**:
   ...
   - **寄り道は刻限60秒のスコアアタック**。stage が `getHudTimerSec()/getScore()/isTimeUp()/sideKind` を持ち、時間切れで `game.beginSideResult()` → `SIDE_RESULT`(結果発表)→セレクト。
   - **最高記録は saveGlobal の `sideBest` に難易度別で持つ**(プレイヤーのセーブとは独立)。キーは `` `${kind}_${difficultyId}` ``(例 `training_hard`)。敵の体力が易0.8〜難1.8倍と幅があり同じ土俵で比べられないため。キーの決定・旧データ(難易度なしの単一値)の「普」への移設・読み出しは `js/sideStageCommon.js` の `sideBestKey`/`normalizeSideBests`/`getSideBest`。**移設が起きたら記録更新が無くても書き戻す**(次回また同じ移設をしないため)。
   - **蔵と道場で同じになる部品は `js/sideStageCommon.js` へ**(獲得の集約 `pushGain`・浮き文字 `updateGainPops`/`renderGainPops`・刻限 `tickTimeLimit`・左端クランプ `clampToLeftEdge`・記録キー)。両クラスへ写すと片方だけ直して必ず食い違う。
   - **寄り道は何度でも挑める**(記録更新が目的なので在庫制にはしない)。**startNewGame は開始時に即セーブする**既存仕様なので、`?map=` デバッグ起動はセーブを上書きする(検証時は注意)。
   - **小判蔵の塔は手で組んだ5つの型のローテーション**(`TOWER_PATTERNS`)。完全ランダム生成は小判が増えすぎ構成の良し悪しも揺れたため差し戻した(実機フィードバック 2026-08-11)。毎回変えるのは「どの型か」と吊り棚の位相だけ。型を足す/直すときは**到達可能性を数値で守る**: 単発ジャンプ=高さ160px/水平240px、2段=282px/384px(GRAVITY 0.8・JUMP_FORCE -16・DOUBLE_JUMP -14・SPEED 6 から算出)。縦の段差は ≤145、横の隙間は ≤215 に収め、動く棚の振幅は壁までの余地でクランプする(下限を `Math.max` で張ると画面外へ出る)。`scratch/design_towers.mjs` で全型を再検証すること。
...
6d. **アセットは正面ビュー固定・小判は縦向き**(世界観のルール。実機フィードバック 2026-08-11):
   - 斜め45度・俯瞰・パースを付けない。生成プロンプトにも「真正面から見た平面的な図／俯瞰・斜め・パースを付けない」を明記する。
   - 「立体感が足りない」を**視点で解決しない**。プロポーション(千両箱は横長 3:2)・陰影・金具の作り込みで解く。
   - 小判は**縦長が標準の向き**。回転・明滅・浮き沈みを足さず、面に接地させて置く。
   - 生成画像は**クロマキー(#00ff00)背景で作り透過へ抜く**。RGBのまま置くと背景に四角い黒が出る(bonus_kura_chest.png で実際に発生)。中間ファイル(*_raw.png)はコミットしない。
